...
<!-- <form class="searchbar" action="" method="GET"> -->
  ...
  <input type="text" class="search-box" id="myInput" name="input" placeholder="Search for post..." title="Type in a title">
  <button type="submit" class="submit" onclick="handleClick();"><i class="fa fa-search"></i></button>
<!-- </form> -->
          ...
          success : function (response){
            ...
            for (var i = 0;i <= result.length - 1; i++) {
              $(`#list`).append(`<tr id="row_${result[i].id}">
                ...
                <td>
                  <a class="btn btn-warning" href="index.php?controller=posts&action=update&id=${result[i].id}">Update</a>
                  <a class="btn btn-danger" data-toggle="modal" data-target="#exampleModal${result[i].id}" href="index.php?controller=posts&action=delete&id=${result[i].id}"> DELETE </a>
                </td>
              </tr>`);
              $(`#list`).append(`<div class="modal fade" id="exampleModal${result[i].id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel">DELETE</h5>
                    ...
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-dismiss="modal">NO</button>
                      <a class="btn btn-primary" href="index.php?controller=posts&action=delete&id=${result[i].id}" >YES</a>
                    </div>
                  </div>
                </div>
              </div>`)
